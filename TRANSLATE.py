...
def process_asc(file_content, db):
    ...
    # --- 改动点 1: 增强版正则，将 Rx 改为 (?P<dir>Rx|Tx) 以兼容附件 1 的发送帧 ---
    frame_re = re.compile(
        r'^\s*(?P<time>\d+\.\d+)\s+'
        r'(?P<channel>\d+)\s+'
        r'(?P<id>[0-9A-Fa-f]+)x?\s+'
        r'(?P<dir>Rx|Tx)\s+d\s+'  # 这里是关键：不再限制死 Rx
        r'(?P<dlc>\d+)\s+'
...
# ===================== 3. UI 交互与逻辑控制 =====================
db = load_dbc()
st.title("🚗 HVFAN 综合分析系统 (Rx/Tx 兼容版)")
...
if not db:
    ...
    if uploaded_file is not None:
        ...
        if 'current_file' not in st.session_state or st.session_state.current_file != file_key:
            with st.spinner('🔍 正在解析 Rx/Tx 报文并加载数据...'):
        ...
        if not full_data:
            st.warning("⚠️ 未能解析到有效信号。请确认 ASC 文件中包含 Rx 或 Tx 报文。")
        else:
    ...
    st.sidebar.divider()
    st.sidebar.caption("HVFAN Tool v18.2 | Direction-Agnostic Engine")
